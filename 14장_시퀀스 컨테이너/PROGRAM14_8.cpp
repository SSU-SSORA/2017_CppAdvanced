...
int main() {
	...
	myList.push_back(std::make_pair(std::string("전구"), 200));
	...
	myList.unique(u);
	print("unique로 중복 삭제 후 myList : ", myList);

	myList.pop_front();
	myList.erase(myList.begin());
	print("pop_front와 erase 함수로 삭제 후 myList : ", myList);
	return 0;
}